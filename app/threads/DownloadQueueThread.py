        ...
        for item in self.queue:
            ...
            try:
                if self.key:
                    ...
                    print(f'queue header: {headers}')
                    response = self.session.get(Settings.endpoint + f'{item[0]}', headers=headers).json()
                else:
                ...
                for i, link in enumerate(links):
                    thread = QThread()
                    worker = DownloadThread(link, item_folder, i, session=self.session, key=self.key)
